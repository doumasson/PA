from pa.plugins import Callback, PluginBase, Command, NLHandler
from pa.plugins.google.jobs import get_google_jobs
from pa.plugins.google.commands import (
    handle_gmail_check, handle_gmail_nl, handle_calendar_nl, handle_email_block,
    handle_email_blocks, handle_email_unblock, handle_kid_sport,
    handle_email_search,
)
from pa.plugins.google.outbox import handle_email_callback, handle_email_nl


async def handle_bill_scan(ctx, text, update) -> str:
    from pa.plugins.google.bills import run_bill_extraction
    return await run_bill_extraction(ctx)
...
"""
    ...
    def callbacks(self) -> list:
        return [
            Callback(prefix="email", handler=handle_email_callback,
                     description="Send/discard staged email drafts"),
        ]

    def nl_handlers(self) -> list:
        return [
            NLHandler(
                keywords=["reply to", "respond to", "email him", "email her",
                          "email them", "send an email", "write an email",
                          "write back", "tell them", "email the",
                          "schedule an appointment", "book an appointment",
                          "set up an appointment", "schedule a meeting",
                          "ask for a time", "reschedule"],
                handler=handle_email_nl,
                description="Draft and send an email or reply (drafts require Send approval)",
                priority=16,
                intent_id="google.email_send",
                examples=["reply to the school email saying we'll be there",
                          "send an email to coach Mike asking about practice time",
                          "write back to the dentist and ask for a Thursday slot"],
            ),
            NLHandler(
            ...
            NLHandler(
                ...
                handler=handle_calendar_nl,
                priority=8,
                intent_id="google.calendar",
                description=(
                    "Check calendar events/appointments/schedule, or delete a "
                    "calendar event by name (including recurring series)"
                ),
                examples=["what's on my calendar today", "delete the discussion due event"],
            ),
        ]
