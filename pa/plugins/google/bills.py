...
EXTRACT_SYSTEM = """Extract bill/statement data from these emails. Return ONLY a JSON object of the form {"results": [...]}, no markdown.

For each email include one entry:
...
async def run_bill_extraction(ctx, days_back: int = 90, notify: bool = True) -> str:
    ...
    try:
        data = await ctx.brain.query_json(
            msg, tier=Tier.PARSE, system=EXTRACT_SYSTEM, max_tokens=4096,
        )
        all_results = data.get('results') or []
    except ValueError:
        # Covers json.JSONDecodeError too — the reply wasn't parseable JSON.
        return f"Could not parse response from {len(unique_emails)} emails."
    except Exception as e:
        if ctx.ledger:
            await ctx.ledger.record(e, source="google.bill_extraction")
