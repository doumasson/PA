...
async def _get_nag_count(ctx) -> int:
    ...
    row = await ctx.store.fetchone(
        "SELECT value FROM meals_state WHERE key = ?", (_STATE_KEY_IGNORED,)
    )
...
async def _increment_nag(ctx) -> None:
    ...
    await ctx.store.execute(
        "INSERT INTO meals_state (key, value) VALUES (?, '1') "
...
async def reset_dinner_nag(ctx) -> None:
    ...
    await ctx.store.execute(
        "INSERT INTO meals_state (key, value) VALUES (?, '0') "
        "ON CONFLICT(key) DO UPDATE SET value = '0'",
        (_STATE_KEY_IGNORED,),
    )
    await ctx.store.execute(
        "INSERT INTO meals_state (key, value) VALUES (?, ?) "
