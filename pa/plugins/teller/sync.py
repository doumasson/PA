...
_PRIMARY_INSTITUTION_MATCH = "wells"
_PRIMARY_INSTITUTION_LABEL = "Wells Fargo"
...
async def sync_teller_accounts(ctx, institutions=None):
    ...
    messages = []
    tokens = {}
    for name in ctx.vault.institutions():
        if not name.startswith('teller_'):
            continue
        creds = ctx.vault.get(name)
        if isinstance(creds, dict) and 'access_token' in creds:
            tokens[name.replace('teller_', '')] = creds.get('access_token')
    if not tokens:
    ...
    for inst_name, token in tokens.items():
        ...
        try:
            ...
            for acct in accounts:
                ...
                try:
                    txns = client.get_transactions(token, acct['id'], count=150)
                    for t in txns:
                        # Teller signs amounts (money out is negative, deposits
                        # positive); the DB stores spending as positive, so flip.
                        ...
                        await repo.add_transaction(
                            ...
                            amount=amount,
                            posted_date=None if pending else t.get('date'),
...
async def get_weekly_spending_summary(ctx):
    ...
    prompt = (
        f"Analyze {_owner(ctx)}'s spending for the past 7 days. Total: ${total:,.2f}\n\n"
...
async def get_yesterday_summary(ctx):
    ...
    wf_balances = [b for b in balances if _PRIMARY_INSTITUTION_MATCH in b['institution'].lower()]
    ...
    prompt = f"Give {_owner(ctx)} a brief friendly morning financial update.\n\n{_PRIMARY_INSTITUTION_LABEL} balances:\n{balance_str}\n\n{txn_str}\n\nKeep it to 3-4 sentences. Speak like Dumbledore. Flag anything concerning."
    return await ctx.brain.complete(prompt, tier=Tier.PARSE, context_id=None)
