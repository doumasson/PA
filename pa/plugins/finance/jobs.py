...
# TODO: primary institution is hardcoded — move to config/profile.
_PRIMARY_INSTITUTION = "wellsfargo"


async def _retry(coro_fn, ctx, max_retries=3, source="finance"):
    """Self-healing retry wrapper. Records errors to the ledger, retries with backoff."""
    last_error = None
    for attempt in range(max_retries):
        try:
            ...
            last_error = e
            if ctx.ledger:
                await ctx.ledger.record(e, source=source)
...
async def detect_recurring_payments(ctx) -> None:
    ...
    # Deposits are negative — they MUST be in the list or income detection
    # sees nothing, and the income estimate silently freezes at its old value
    # (raises would never register).
    ...
    lines += [
        f"{t['date']} {t['description'][:50]} +${-t['amount']:,.2f} DEPOSIT"
        for t in credits[:40]
    ]
    txn_list = "\n".join(lines)
...
Return ONLY JSON:
{
  ...
  "spending_concerns": ["list of specific spending patterns that seem high or wasteful"]
    ...
    try:
        # 4 lists over ~140 transactions overflow the default 1024 max_tokens,
        # truncating the JSON — this job silently permafailed on that once before.
        data = await ctx.brain.query_json(
            txn_list, tier=Tier.PARSE, system=SYSTEM, max_tokens=4096
        )
        ...
        # Monthly income estimate — regular sources only; 'irregular' entries
        # are cash advances (Brigit/Cleo) and one-offs, which are loans, not pay.
        if income:
            monthly_income = sum(
                i['amount'] * (2 if i.get('frequency') == 'biweekly' else 1)
                for i in income
                if i.get('frequency') in ('biweekly', 'monthly')
            )
            if monthly_income > 0:
        ...
        if ctx.ledger:
            await ctx.ledger.record(e, source="detect_recurring")
...
async def job_morning_sync(ctx) -> None:
    ...
    async def _do_sync():
        ...
        # Refresh recurring/income detection on Mondays — or whenever the
        # last successful run is stale, so one failed Monday doesn't leave
        # income and subscriptions frozen for weeks.
        from pa.plugins.finance.advisor import load_profile
        last = (await load_profile(ctx)).get('recurring_updated')
        stale = True
        if last:
            try:
                stale = (datetime.date.today()
                         - datetime.date.fromisoformat(str(last).strip('"'))).days > 10
            except ValueError:
                pass
        if stale or datetime.date.today().weekday() == 0:
...
async def job_balance_check(ctx) -> None:
    ...
    async def _do_check():
        from pa.plugins.teller.sync import sync_teller_accounts
        from pa.plugins.finance.repository import FinanceRepository
        await sync_teller_accounts(ctx, institutions=[_PRIMARY_INSTITUTION])
        repo = FinanceRepository(ctx.store)
        balances = await repo.get_latest_balances()
        checking = [b for b in balances if b['type'] in ('checking', 'savings', 'depository')]
        for acct in checking:
            # Alert once per low-balance VALUE: restarts and 4h re-checks stay
            # silent until the balance moves; recovery re-arms the alert.
            state_key = f"low_balance_alerted:{acct['institution']}:{acct['name']}"
            if acct['balance'] < 200:
                current = f"{acct['balance']:.2f}"
                row = await ctx.store.fetchone(
                    "SELECT value FROM core_state WHERE key = ?", (state_key,)
                )
                ...
                await ctx.store.execute(
                    "DELETE FROM core_state WHERE key = ?", (state_key,)
                )
                ...
                try:
                    ...
                    if 0 <= days_until <= 3:
                        urgent.append(
                            ...
                            f"${b.get('minimum_payment') or b.get('statement_balance') or b['balance']:,.2f} due in {days_until} day(s)"
                        )
                except Exception:
                    pass
        if urgent:
...
async def _roll_recurring_bills(ctx) -> None:
    """Advance past-due recurring bills to their next cycle (new cycle = unpaid)."""
    import calendar
    import datetime
    ...
    months_step = {"monthly": 1, "quarterly": 3, "yearly": 12, "annual": 12}
    weeks_step = {"weekly": 1, "biweekly": 2}
    today = datetime.date.today()
    bills = await ctx.store.fetchall(
        "SELECT id, due_date, frequency FROM finance_bills "
        "WHERE due_date IS NOT NULL AND due_date < ?",
        (today.isoformat(),),
    )
    for b in bills:
        freq = (b["frequency"] or "").lower()
        if freq not in months_step and freq not in weeks_step:
            continue  # one-time bill — leave for manual cleanup
        try:
            due = datetime.date.fromisoformat(b["due_date"])
        except ValueError:
            continue
        while due < today:
            if freq in weeks_step:
                due += datetime.timedelta(weeks=weeks_step[freq])
            else:
                due = add_months(due, months_step[freq])
        await ctx.store.execute(
            "UPDATE finance_bills SET due_date = ?, paid_this_cycle = 0, "
...
async def job_bill_reminders(ctx) -> None:
    ...
    async def _do_bill_reminders():
        ...
        await _roll_recurring_bills(ctx)
        today = datetime.date.today()
...
async def job_bart_daily_nag(ctx) -> None:
    ...
    async def _do_nag():
        ...
        # Unpaid bills due in the next 45 days (roll first so dates are current)
        await _roll_recurring_bills(ctx)
        bills_cutoff = (datetime.date.today() + datetime.timedelta(days=45)).isoformat()
...
async def job_money_guardian(ctx) -> None:
    """Daily at 12:30 — Money Guardian anomaly sweep (pure SQL, zero API calls).
    Duplicate charges, subscription price creep, big first-time merchants."""
    if not ctx.vault.is_unlocked:
        return

    async def _do_guardian():
        from pa.plugins.finance.guardian import job_guardian_check
        await job_guardian_check(ctx)

    await _retry(_do_guardian, ctx, source="guardian_check")


async def job_bill_mastery(ctx) -> None:
    """Saturday 09:30 — bill discovery sweep (pure SQL, zero LLM calls).
    Back-fills missing bill due dates and proposes untracked recurring
    charges as one-time Track/Dismiss approvals."""
    if not ctx.vault.is_unlocked:
        return

    async def _do_discovery():
        from pa.plugins.finance.billmaster import job_bill_discovery
        await job_bill_discovery(ctx)

    await _retry(_do_discovery, ctx, source="bill_discovery")
...
def get_finance_jobs() -> list[Job]:
    return [
        ...
        Job(name="guardian_check", handler=job_money_guardian,
            trigger="cron", kwargs={"hour": 12, "minute": 30}),
        Job(name="bill_discovery", handler=job_bill_mastery,
            trigger="cron", kwargs={"day_of_week": "sat", "hour": 9, "minute": 30}),
    ]
