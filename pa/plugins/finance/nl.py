...
async def handle_finance_nl(ctx: AppContext, text: str, update: Update) -> str:
    ...
    _kid_names = {"the boys", "the kids"}
    if ctx.profile:
        _kid_names |= {k.name.lower() for k in ctx.profile.kids}
    ...
    if any(phrase in tl for phrase in [
        ...
        # Use the parse tier to parse the correction
...
If unclear: {"action":"unclear"}. Raw JSON only."""
    ...
    if any(w in tl for w in ["balance", "how much", "account", "checking", "savings", "credit card"]):
        ...
        return await ctx.brain.complete(prompt, tier=Tier.PARSE, context_id=None)

    elif any(w in tl for w in ["debt", "owe", "loan", "mortgage", "payoff"]):
        from pa.plugins.finance.advisor import _debts_not_already_stored, ADVISOR_SYSTEM_TEMPLATE
        balances = await repo.get_latest_balances()
        ...
        # Manual rows are authoritative; only add Teller/Gmail accounts that
        # aren't already one of them, so nothing is counted twice.
        extra = _debts_not_already_stored(debts, stored)
        lines = []
        for d in stored:
        ...
        for d in extra:
            ...
            lines.append(line)
        total = sum(d['balance'] for d in stored) + sum(d['balance'] for d in extra)
        data_summary = "\n".join(lines)
        ...
        return await ctx.brain.complete(
            prompt, tier=Tier.PARSE, context_id=None,
            system=ADVISOR_SYSTEM_TEMPLATE.format(owner=owner),
        )
        ...
        data_summary = "\n".join(
            ...
            + (f"${d.get('minimum_payment') or d.get('statement_balance') or d['balance']:,.2f} due {d['due_date']}"
               if d.get('due_date') else f"${d['balance']:,.2f}")
            for d in due
        )
...
async def handle_affordability_nl(ctx: AppContext, text: str, update: Update) -> str:
    ...
    # Use the parse tier to extract the dollar amount from the message
...
Raw JSON only."""
    ...
    # Only bills due before the next paycheck (~14 days) count against what's
    # spendable now — counting every unpaid bill of the month made everything
    # look unaffordable.
    upcoming_bills = await ctx.store.fetchall(
        "SELECT name, amount FROM finance_bills "
        "WHERE paid_this_cycle = 0 AND amount IS NOT NULL "
        "AND (due_date IS NULL OR due_date <= date('now', '+14 days'))"
    )
