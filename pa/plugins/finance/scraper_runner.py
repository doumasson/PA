...
async def _create_pilot(data_dir: str) -> tuple:
    ...
    # scrub_untrusted is off in the scraper subprocess: the page HTML is
    # exactly what we're analyzing, and credentials are already
    # placeholder-protected by the pilot's own $cred mechanism; scrubbing
    # would corrupt selectors.
    brain_config = {
        "llm": {
            "backends": {
                "cliproxy": {
        ...
        if cookies:
            ...
            try:
                check = await pilot._brain.query_json(
                    "Does this page show account balances or a dashboard (not a login form)? "
                    "Respond with {\"logged_in\": true/false}.\n\nHTML:\n" + html,
                    tier=Tier.PARSE,
                    system="You are checking if a browser session is still valid. Look at the HTML and determine if this is a logged-in dashboard/accounts page or a login page.",
                )
