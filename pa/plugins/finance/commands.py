...
async def handle_spending(ctx: AppContext, update: Any, context: Any) -> str:
    ...
    elif "30" in pl or "month" in pl and "this" not in pl:
        since = (today - datetime.timedelta(days=30)).isoformat()
    else:  # "this month" default
        since = today.replace(day=1).isoformat()
    txns = await _repo(ctx).get_transactions(since_date=since, limit=500)
...
async def handle_plan(ctx: AppContext, update: Any, context: Any) -> str:
    ...
    # Route through the advisor so the plan sees his real balances, the owner
    # debt strategy (deliberately charged-off accounts), and Bart's persona —
    # otherwise the model invents numbers and nags about the charge-offs.
    from pa.plugins.finance.advisor import run_advisor
    return await run_advisor(
        ctx,
        user_question="Create a debt payoff plan based on my current balances. "
        "Compare snowball vs avalanche, but respect my stated debt strategy.",
    )
...
async def handle_budget(ctx: AppContext, update: Any, context: Any) -> str:
    ...
    for b in budgets:
        ...
        # Match the budget category against both the exact category and any
        # hierarchical child ("Food" covers "Food/Dining", "Food/Delivery").
...
async def handle_recat(ctx: AppContext, update: Any, context: Any) -> str:
    ...
    if still_unknown:
        ...
        SYSTEM = (
            "Categorize these merchant/transaction descriptions. Return ONLY a JSON object:\n"
            '{"items": [{"description": "...", "category": "Groceries|Gas|Food/Dining|Food/Delivery|Food/Coffee|'
            'Shopping|Liquor|Software|Subscription|Bills|Transfer|Recreation|Medical|Auto|Income|Kids|Cash Advance|Other"}]}\n'
            "Raw JSON only."
        )
        try:
            data = await ctx.brain.query_json(desc_list, tier=Tier.PARSE, system=SYSTEM)
...
async def handle_guardian(ctx: AppContext, update: Any, context: Any) -> str:
    """/guardian — run Money Guardian anomaly checks on demand.

    Reports all current findings, marking ones already alerted. Does not
    record fingerprints — only the scheduled job consumes alert slots.
    """
    from pa.plugins.finance.guardian import get_alerted_fingerprints, run_checks

    findings = await run_checks(ctx.store)
    if not findings:
        return "\U0001f6e1 Money Guardian: all clear — no anomalies found."
    seen = await get_alerted_fingerprints(ctx.store)
    lines = ["\U0001f6e1 Money Guardian findings:"]
    for f in findings:
        mark = " (already alerted)" if f["fingerprint"] in seen else ""
        lines.append(f"• {f['message']}{mark}")
    return "\n".join(lines)
