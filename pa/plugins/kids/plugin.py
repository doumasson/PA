"""Kid Activity Tracker plugin.

Kid names/ages come from ctx.profile.kids at runtime; the NL routing
keywords below still name the kids because no ctx exists at registration
time (update config + this list together if the family changes).
"""
...
class KidsPlugin(PluginBase):
    name = "kids"
    description = "Kid activity tracker — events, practices, games, and notes per kid"
    version = "0.1.0"
    ...
    def system_prompt_fragment(self) -> str:
        return "Kid activity tracker active (events, practices, games, notes per kid)."
